...
5. **Display swap:** the resting placeholder (see §5) swaps for the real player + cards on first serve. **Card anatomy:** number `01` → the question (the hook: *what this moment answers*) → footer: subtopic tag + duration. **Now Playing line (decluttered, Tommy 6 Jul):** duration + episode-exit link ONLY — `1m 18s · From EP 266 →` (no "Now playing NN", no "watch the full episode" — the card highlight shows what's playing and the link styling makes the exit clear), with the moment's `description` beneath in white (context once committed).
