...
- **EP 263 is the FIRST live episode-page template (v2, approved-in-progress).**
  ...
  - `index.html` = the homepage, rebuilt 6 Jul late evening as ONE SCREEN: video banner with a play-once title sequence (logo pop → "5 years." → "150+ hours. 280+ episodes." → show lockup HOLDS) + pulsing scroll cue → WAYS TO EXPLORE THE SHOW (three coloured cards that pop in staggered, each carrying a LIVING PREVIEW in a 16:9 window: mint Latest episode = full uncropped thumbnail + dynamic "Watch EP {n}" button, auto-updates per release; purple Choose your own adventure = 18s acted-out demo — pick a topic → three chips invert to selected purple → dig deeper → two subchips pressed → Show me the moments button clicks, all real inventory from search-index.json, beats fade as one, loop-wrap glitch-proofed; black Explore every episode = wall of thumbnails drifting down in 3 columns → YouTube channel until episodes.html exists. Mobile: adventure window grows to 290px so the 12 topic chips never clip) → footer. **LOCKED (Tommy): the video banner's bottom fade-out (mask-image, solid to 68%) must NEVER be removed.** No about section (lockup carries it); BTS photo band parked awaiting a photo from Tommy.
